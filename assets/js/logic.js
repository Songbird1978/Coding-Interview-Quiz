...
let answer = document.querySelector("#answerbutton"); //linking new variable to the id of answer (question button)
...
startQuiz.addEventListener("click", function(){ //creating an event listener for the click which runs a function
    ...
    for (let i = 0; i < questionData[0].choices.length; i++){ //for loop looking at each element of the questionData array and looping through until the end 
        ...
        button.textContent = questionData[0].choices[i]; //linking the text of the question (questionData array) and the choices div to the answer buttons 
        ...
        var soundCorrect = document.getElementById()
        answer.addEventListener("click", function(){ //add function for click answer
            if (button.textContent.answer !== questionData[0].question) {
